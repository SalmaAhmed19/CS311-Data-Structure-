...
#include<cmath>
#include <iostream>
// CS311 YOSHII S22 binsearch.cpp
...
int binarySearch(int L[], int x, int first, int last) 
{
  ...
  if(first==last && x!=L[first]){//when number is not found 
      return -1;
    }
  if(first==last){//to handle the test of 1 and not returning slot 0 
      return first;
    }
...
{ int A[10];
  ...
  //**
  int respos = binarySearch(A,e,myFirst,myLast);//format in the notes: myList, findthis,myfirst,mylast(A os the list,find e , first element is 0 , last element is 9
